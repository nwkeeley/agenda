...
var Agenda = module.exports = function(config) {
  ...
  this._maxJobsPerWorker = config.maxJobsPerWorker || 100;
  this._definitions = {};
  this._runningJobs = [];
  this._jobQueue = [];
...
function processJobs(extraJob) {
  ...
  if(this._runningJobs.length > 0) return; 
  ...
  function jobQueueFilling(name, i) {
    var now = new Date();
    ...
    self._findAndLockNextJob(name, definitions[name], function (err, job) {
      ...
      if(job) {
        ...
        if(i<self._maxJobsPerWorker) {
          jobQueueFilling(name, i);          
        }
        jobProcessing();
      }
    });
